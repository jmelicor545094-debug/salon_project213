<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">{{ __('New Appointment') }}</h2>
                ...
                <form method="POST" action="{{ route('appointments.store') }}">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <select id="service_id" name="service_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-900 dark:border-gray-700 dark:text-gray-100" required>
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" @selected(old('service_id') == $service->id)>{{ $service->name }} — ₱{{ number_format($service->price, 2) }} / {{ $service->duration }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="flex items-center gap-4">
                            <x-primary-button>{{ __('Save Appointment') }}</x-primary-button>
